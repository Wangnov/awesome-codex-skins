...
要求：macOS、Node ≥ 20、官方 Codex 应用（`Codex.app` 或品牌合并后的 `ChatGPT.app`）。非标准安装位置可通过 `CODEX_APP_PATH=/path/to/Codex.app` 指定。Windows 支持随 [Codex App Manager](https://github.com/Wangnov/Codex-App-Manager) 演进。
...
Requirements: macOS, Node ≥ 20, and the official Codex app (`Codex.app` or the post-rebrand `ChatGPT.app`). Set `CODEX_APP_PATH=/path/to/Codex.app` for a nonstandard install location. Windows support tracks [Codex App Manager](https://github.com/Wangnov/Codex-App-Manager).
